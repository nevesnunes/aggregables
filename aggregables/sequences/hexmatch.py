...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--regex", default=False, action='store_true', help="parse needle as regular expression")
    parser.add_argument("-o", "--output", default='bytes', type=str, help="encoding used for results output (e.g. latin-1)")
    ...
    for i in range(-k, k + 1, 1):
        for e in endianness:
            ...
            for offset in offsets:
                ...
                results.append(f"{highlight_bold(filename)}:{highlight_primary(offset[0])}({highlight_primary(hex(offset[0]))}):{highlight_bold(iteration)}{':' if iteration else ''}{binascii.hexlify(matched_bytes).decode('ascii')} {clean_match(matched_bytes, parsed_args.output)}")
